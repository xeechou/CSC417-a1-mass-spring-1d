...
template <typename FORCE> inline void symplectic_euler(Eigen::VectorXd &q, Eigen::VectorXd &qdot, double dt, double mass, FORCE &force) {
    Eigen::VectorXd f;
    force(f, q, qdot);
    qdot = qdot + f / mass * dt;
    q    = q + qdot * dt; // this is symplectic_euler actually.
}
